...
function App() {
    const [screen, setScreen] = useState(() => localStorage.getItem("luca_token") ? "home" : "login");
    ...
    useEffect(() => {
        if (screen === "materials" && user?.role !== "admin") setScreen("home");
    }, [screen, user]);
    ...
    const handleMaterialUpload = async (event) => {
        ...
        const selected = subjects.find((item) => item.id === materialForm.subjectId);
                        ...
                        <div className="material-form-grid">
                            <label>Subject
                                <select value={materialForm.subjectId} onChange={(e) => { const id=e.target.value; const s=subjects.find(x=>x.id===id); setMaterialForm({ subjectId:id, unit:"Unit 1", topic:s?.lessons?.[0] || "" }); }}>
                                    {subjects.map((item) => <option value={item.id} key={item.id}>{item.code} — {item.name}</option>)}
                                </select>
                            </label>
                            <label>Unit / Module
                            ...
                            <label>Topic
                                <select value={materialForm.topic} onChange={(e) => setMaterialForm({ ...materialForm, topic: e.target.value })}>
                                    {(subjects.find((item) => item.id === materialForm.subjectId)?.lessons || []).map((item) => <option value={item} key={item}>{item}</option>)}
                                </select>
                            </label>
                            <label>Material file
