...
await initMaterialsStore();
const tokenize = (value) => clean(value, 1000).toLowerCase().replace(/[^a-z0-9\s-]/g, " ").split(/\s+/).filter((x) => x.length > 2);

async function initMaterialsStore() {
  if (!curriculumPool) {
    await fs.mkdir(materialsDir, { recursive: true });
    try { await fs.access(materialsIndexPath); }
    catch { await fs.writeFile(materialsIndexPath, JSON.stringify({ materials: [] }, null, 2), "utf8"); }
    return;
  }

  await curriculumPool.query(`
    CREATE TABLE IF NOT EXISTS luca_materials (
      id TEXT PRIMARY KEY,
  ...
  // One-time migration of materials uploaded by the old JSON-based version.
  // This keeps existing materials instead of silently losing them when moving to Postgres.
  try {
    const count = await curriculumPool.query("SELECT COUNT(*)::int AS count FROM luca_materials");
    if (count.rows[0].count === 0) {
      const legacy = JSON.parse(await fs.readFile(materialsIndexPath, "utf8"));
      for (const material of legacy.materials || []) {
        await curriculumPool.query(
          `INSERT INTO luca_materials (id, filename, subject, unit, topic, mime_type, uploaded_at, chunks)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb) ON CONFLICT (id) DO NOTHING`,
          [
            String(material.id),
            clean(material.filename, 255),
            clean(material.subject, 200),
            clean(material.unit || "", 100),
            clean(material.topic, 200),
            clean(material.mimeType || "text/plain", 100),
            material.uploadedAt || new Date().toISOString(),
            JSON.stringify(Array.isArray(material.chunks) ? material.chunks : []),
          ]
        );
      }
      if ((legacy.materials || []).length) {
        console.log(`📚 Migrated ${legacy.materials.length} legacy material(s) to PostgreSQL.`);
      }
    }
  } catch (error) {
    // A missing/invalid legacy file should not prevent the database-backed app from starting.
    console.warn("Legacy materials migration skipped:", error.message);
  }
}
...
async function readMaterialIndex() {
  if (curriculumPool) {
    const result = await curriculumPool.query(`
      SELECT id, filename, subject, unit, topic, mime_type AS "mimeType",
             uploaded_at AS "uploadedAt", chunks
      FROM luca_materials
...
async function writeMaterial(material) {
  if (curriculumPool) {
    await curriculumPool.query(
      `INSERT INTO luca_materials
       (id, filename, subject, unit, topic, mime_type, uploaded_at, chunks)
      ...
      [
        ...
        material.mimeType,
        material.uploadedAt,
        JSON.stringify(material.chunks),
      ]
    );
    return;
  }
...
async function deleteMaterial(id) {
  if (curriculumPool) {
    const result = await curriculumPool.query("DELETE FROM luca_materials WHERE id = $1", [id]);
    return result.rowCount > 0;
  }
  const index = await readMaterialIndex();
  const before = index.materials?.length || 0;
  index.materials = (index.materials || []).filter((item) => item.id !== id);
  if (index.materials.length === before) return false;
  await fs.writeFile(materialsIndexPath, JSON.stringify(index, null, 2), "utf8");
  return true;
}
